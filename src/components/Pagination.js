...
export const Pagination = ({pagination, handleGotoPage}) => {
	...
	return <div className="pagination">
		{pagination.pageNo > 1 && <>
			<buttton className="page" onClick={()=>{handleGotoPage(1)}}>
				<ion-icon name="play-skip-back-outline"></ion-icon>
			</buttton>
			<buttton className="page" onClick={()=>{handleGotoPage(pagination.pageNo-1)}}>
		...
		{pageNumbers.map((pn, index) => <>
				...
				<buttton onClick={()=>{handleGotoPage(pn)}}
					className={`page ${pagination.pageNo == pn ? 'current' : ''}`}>{pn}</buttton>
			</>
		)}
		{pagination.pageNo < pagination.totalPages && <>
			...
			<buttton className="page" onClick={()=>{handleGotoPage(pagination.totalPages)}}>
