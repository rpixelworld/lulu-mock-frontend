...
export const Breadcrumb = () => {
  ...
  const productCatagories = useSelector(
    (state) => state.productReducer.productCatagories,
  );
  const templateFilters = useSelector(
    (state) => state.productReducer.templateFilters,
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      for (let i = 0; i < catArr.length; i++) {
        if (
          i == 0 &&
          (catArr[0].includes("Men") || catArr[0].includes("Women"))
        ) {
          let index = Breadsrumb_CatagoryIndex["Gender"][catArr[0]];
          recommendFilters["Gender"][index].isChecked = true;
        } else {
          let index =
            Breadsrumb_CatagoryIndex["Catagory"][catArr[catArr.length - 1]];
