...
const OrderSummary = () => {
    ...
    const totalCost = summary.totalCost; // Use the real total cost from the state

    const province = 'bc'; // Example province
    // const province = useSelector((state) => state.shoppingReducer.province);

    const [taxAmount, setTaxAmount] = useState(0);
    const handleTaxAmountChange = (tax) => {
        setTaxAmount(tax);
    };
    const total = (totalCost + shippingFee + taxAmount).toFixed(2);
        ...
        <div className="order-summary">
            <div className="summary-title">
                ...
                <div className="summary-accordion">
                    ...
                    <span className="summary-price title-price">
            ${summary.totalCost}.00
          </span>
                </div>
            </div>
            <hr/>
                        ...
                        {summary?.items?.map((item, index) => (
                            <div key={index} className="summary-item">
                                ...
                                <div className="summary-item-total-price">
                                    <span className="summary-price">${item.price}.00</span>
                                </div>
                            </div>
                        )) || []}
                    </div>
            ...
            <div className="summary-final">
                ...
                <div className="final-list">
                    <span>Tax</span>
                    <span>${taxAmount.toFixed(2)}</span> {/* Display the tax amount here */}
                    <TaxCalculator totalCost={totalCost} province={province} onTaxAmountChange={handleTaxAmountChange}/>
                </div>
            </div>
            <hr/>
