...
export const FeaturePanel = ({ product }) => {
  const [data, setData] = useState(
    product.featurePanels.map((panel) => ({ ...panel, expanded: false })),
  );
                  ...
                  <div>
                    {panel.content.length > 0 ? (
                      <div>
                        <h2>Materials</h2>
                        <ul>
                          {parseProductData(panel.content).material.map(
                            (matItem, matIdx) => (
                              <li className={"list-style"} key={matIdx}>
                                {matItem}
                              </li>
                            ),
                          )}
                        ...
                        <h2>Care</h2>
                        <ul>
                          {parseProductData(panel.content).care.map(
                            (careItem, careIdx) => (
                              <li className={"list-style"} key={careIdx}>
                                {careItem}
                              </li>
                            ),
                          )}
