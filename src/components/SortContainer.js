...
export const SortContainer = () => {
  ...
  const handleAnchorClick = (index) => {
    ...
    for (let i = 0; i < 2; i++) {
      if (i !== index) {
        anchorRefs.current[i].className = anchorRefs.current[
          i
  ...
  const selectSortBy = (e) => {
    ...
    setSortDropDownDisplay((prev) => !prev);
    if (
      e.target.textContent == "Price: High to Low" ||
      e.target.textContent == "Price: Low to High"
    ) {
    ...
    <div className="sort-container">
      <div className="row">
        ...
        <div className="sorting-wrapper">
          ...
          <div
            className={
              sortDropDownDisplay ? "custom-dropdown" : "custom-dropdown hide"
            }
          >
            <ul>
