import "../assets/css/ProductAddToBag.scss";
import Constants from "../Constants";
import React, { useState } from "react";

export const ProductAddToBag = ({ product }) => {
  const [showCarousel, setShowCarousel] = useState(false);
  const colorsPerPage = 7;
  const [currentPage, setCurrentPage] = useState(0);
  const [currentColorIndex, setCurrentColorIndex] = useState(0);
  const [currentBg, setCurrentBg] = useState(
    product.images[currentColorIndex].mainCarousel.media.split(" | ")[0],
  );
  ...
  const switchColor = (cIndex) => {
    ...
    setCurrentLink(
      `${Constants.LOCAL_BASE_URL}/product/${product.productId}?color=${product.images[cIndex].colorId}`,
    );
  };
  ...
  const handleNextPage = () => {
    setCurrentPage((prevPage) =>
      Math.min(
        prevPage + 1,
        Math.ceil(product.swatches.length / colorsPerPage) - 1,
      ),
    );
  ...
  const displayPalettes = product.swatches.slice(
  ...
  return (
    <li
      ...
      onMouseEnter={toggleCarousel}
      onMouseLeave={toggleCarousel}
    >
      <div className="product-tile">
        ...
        {showCarousel && (
          <div className="palette-container">
              ...
              {displayPalettes.map((swatch, cIndex) => (
                ...
                >
                  <a href={currentLink}>
                    <img
                      className={
                        currentColorIndex == cIndex ? "active-color" : ""
                      }
                      src={swatch.swatch}
            ...
            {product.swatches.length >= 8 &&
              currentPage <
                Math.floor(product.swatches.length / colorsPerPage) && (
                <button
                  onClick={handleNextPage}
                  className="carousel-button next-button"
                  disabled={
                    (currentPage + 1) * colorsPerPage >= product.swatches.length
                  }
                >
                  &gt;
                </button>
