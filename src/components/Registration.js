...
const Registration = () => {
	...
	const isFormValid=() =>{
		return formData?.firstName?.length>0 && formData?.lastName?.length>0 && formData?.age?.length>0 && formData?.email?.length>=5 && formData?.password?.length>=6
	}
	...
	return <div className="regi-basebord">
		<div className='side-face'></div>

		{isSuccess
			? <div className="success-signup">
				<div className="logo">
					<img src="https://upload.wikimedia.org/wikipedia/commons/2/22/Lululemon_Athletica_logo.svg"
						 alt="logo" />
				</div>
				<h1>Congratulations! <br />You created an account successfully.</h1>
			</div>
			: <div className="registration">
				<div className="signup-box">
					...
					<form onSubmit={submitHandler}>
						...
						{!state.isAge &&
							<p className="regi-err">Age can not be empty, and the number is allowed between 1~150. Try
								again!</p>}
						...
						{!state.isPassword &&
							<p className="regi-err">The password must be at least 6 characters long or longer. Try
								again!</p>}
						<div className="policy">
							<p>By clicking "Create Member Account" you agree to the Terms of Use and to join lululemon
								Membership. See our Privacy Policy for details about our information practices.
								California
								consumers, also see our Notice of Financial Incentives. lululemon will use information
								you
								submit (including identifiers, commercial information, and internet or other electronic
								network activity information) to fulfill this request.</p>
						</div>
						<button type="submit"
								className={isAllTrue && isFormValid ? 'already' : 'not-ready'}
								disabled={!(isAllTrue && isFormValid)}>SIGN UP</button>
					</form>
				</div>
			</div>}
