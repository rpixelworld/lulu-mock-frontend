...
export const ChildMenus = ({
	...
	if (display) {
		...
		return (
			<div className="childmenus-container" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
				{/*{title}*/}
				<div className="childmenus-wrapper">
					<div className="child-menus">
						{featureMenus && featureMenus.length > 0 && (
							<div className="feature-menus">
								{featureMenus.map(fmenu => (
									<a key={fmenu.id} href={fmenu.url}>
						...
						{catagories.map(catagory => {
							...
							let menuClass =
								catagoryMenus.length > 10
									? 'catagory-submenus two-column'
									: 'catagory-submenus one-column';
							return (
								<>
									<div className="catagory-menus">
										...
										<div className={menuClass}>
											{catagoryMenus.map(catMenu => (
												<a href={catMenu.url}>
													<span>{catMenu.name}</span>
												</a>
											))}
										</div>
									</div>
