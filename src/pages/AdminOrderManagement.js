import '../assets/css/AdminOrderManagement.scss';
...
import { NoIconHtmlTooltip } from '../components/NoIconHtmlToolTip';
...
export const AdminOrderManagement = () => {
	...
	const fetchAndSetOrders = (pn = 1, paramFilter) => {
		...
		fetch(`${Constants.BACKEND_BASE_URL}/orders/all?pageNo=${pn}&pageSize=${pagination.pageSize}`, options)
			.then(resp => resp.json())
			.then(result => {
